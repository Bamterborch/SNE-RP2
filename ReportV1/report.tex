...
But for a session oriented reliable protocol such as TCP - which ensures that the data sent is actually received by the target like a Data Transfer Node (DTN), it is much more complex to achieve. For session-based traffic the techniques implemented in a standards-compliant TCP stack make it harder to overload a device in terms of raw bandwidth.
...
The report demonstrates that the weakest links in the path can be identified and its load tolerance limits can be found through these use cases.
\end{abstract}
\end{center}
\end{titlingpage}
